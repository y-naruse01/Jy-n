...
\section{はじめに}
本章では、本研究の導入として、研究の背景、課題、および目的について述べる。まず、都市鉄道における需要予測の現状と課題を整理する。次に、本研究が解決しようとする具体的な問題点を提示し、その解決に向けたアプローチと本研究の貢献について述べる。

\subsection{研究背景}
都市部における鉄道網は、市民の移動を支える極めて重要な社会インフラである。日々の安定輸送を確保するため、鉄道事業者にはダイヤ設定や混雑緩和のための要員配置、臨時列車の運行といった高度な運用判断が求められる。これらの判断を適切に行うためには、近い将来の旅客需要（乗降者数や流動）を正確に見積もるだけでなく、その増減がなぜ生じたのかという原因を現場レベルで即座に把握できることが不可欠である。

近年、SuicaやPASMOに代表される交通系ICカードの普及により、ICカード乗降データが電子的に蓄積されるようになった。これにより、駅単位かつ分単位・時間単位での高精細なデータが入手可能となり、データ駆動型の分析環境が急速に整いつつある。時系列解析や機械学習を用いた需要予測の研究も進展し、過去のトレンドや周期性から将来の需要を高精度に予測することは技術的に可能となってきた。

しかし、鉄道は単なる独立した駅の集合体ではなく、路線網として相互に影響し合うネットワークでもある。ある駅で発生したイベントや突発的な需要変動は、列車の移動や乗換行動を介して、隣接駅やハブ駅へと連鎖的に波及する。例えば、大規模なコンサートが開催された場合、最寄駅の混雑はもちろんのこと、その路線の乗換駅や数駅離れたターミナル駅においても通常とは異なる需要変動が観測される。また、降雨や降雪、猛暑といった気象条件（外生要因）も、通勤・通学以外の外出行動を抑制したり、逆に地下街直結駅への利用を集中させたりと、複雑な影響を及ぼす。

\subsection{課題}
このように、鉄道需要の変動要因は気象などの外生要因と駅間ネットワークを介した波及要因が重畳して構成されている。現場の運用担当者が適切な対応策（例えば、改札制限の要否や駅員配置の増強など）を講じるためには、単に1時間後に利用者が増えるという予測値だけでは不十分であり、近隣駅でのイベント終了に伴う流入が見込まれるため増えるのか、降雨により他路線からの振替輸送が発生しているため増えるのかといった、変動の理由が不可欠である。

しかし、既存の研究やシステムには、この予測と説明の間に大きな乖離が存在するという課題がある。 第一に、予測モデルのブラックボックス性である。近年、グラフ畳み込みネットワーク（GCN）などの深層学習技術を用いることで、駅間の空間的な依存関係を考慮した高精度な予測が可能となった。しかし、これらのモデルは予測性能が高い反面、その出力根拠を人間が解釈することは困難である。 第二に、説明生成技術との分断である。大規模言語モデル（LLM）の発展により、データから自然言語による説明を生成する試みは始まっているが、従来のLLMは一般的な知識に基づいて回答を生成する傾向があり、鉄道固有のネットワーク構造や、その日その時の具体的な数値変動と整合した説明を行うことが難しい。

結果として、予測システムが需要増を警告したとしても、その原因が気象起因なのか、他駅からの波及なのかの切り分けが難しく、運用担当者は自らの経験と勘に頼って判断せざるを得ない状況が残されている。予測精度と説明の妥当性を同一の枠組みで扱い、両者の整合を担保する設計が十分になされていないことが、データ活用を実運用へ定着させる上での大きな障壁となっているのである。

\subsection{研究の目的}
以上の背景を踏まえ、本研究の目的は、気象要因と駅間依存を根拠として、旅客需要の増減要因を自然言語で説明できる統合的な枠組みを構築することである。単に数値を予測するだけでなく、その数値が導き出された背景にある「気象条件」と「近隣駅からの影響波及」を明示的に根拠として提示することで、運用担当者の意思決定を支援することを目指す。

この目的を達成するため、本研究では具体的に以下を課題として設定し、解決を図る。物理的な路線図上の隣接関係だけでなく、実際の人の流れ（ODデータ）に基づく駅間の相関関係を「機能的フローネットワーク」として定義し、これを説明の根拠として利用可能な形式でモデルに組み込む。GCNが抽出する駅間の近傍集約特徴（ネットワーク波及の数値的表現）と、降雨・気温などの外生要因を、予測モデルの入力としてだけでなく、LLMへのプロンプト（指示入力）としても矛盾なく接続するアーキテクチャを設計する。予測性能（MSE等）の評価に加え、生成された説明がどの程度妥当であるかを検証するため、要因の正解ラベルを付与可能な模擬データを用いた定量的評価系を構築し、提案手法の有効性を検証する。


\subsection{貢献}
提案フレームワークは、大きくネットワーク構築、需要予測、説明生成の三つのフェーズから構成される。 まず、ICカードデータから駅間の時系列相関が高い駅同士を抽出し、機能的な結びつきを表すグラフ（機能的フローネットワーク）を構築する。これにより、物理的には離れていても利用者の移動パターンが似通っている駅間の依存関係を捉える。 次に、このグラフ上でGCNを動作させ、近隣駅の混雑状況を集約した特徴量と、気象情報を合わせて需要予測を行う。 最後に、LLM Agentを用い、予測された数値変動に対して、その主要因が気象（雨による需要減など）にあるのか、近接駅（隣の駅の急増に連動した波及）にあるのかを推論し、根拠付きのレポートとして自然言語で出力する。 本研究の貢献は以下にまとめられる。 予測（GCN）と説明（LLM）を分離せず、同一のフレームワーク内で接続する設計を提示した点。これにより、予測モデルが捉えた特徴をそのまま言語化の根拠として用いることが可能となった。 ラグ付き相互相関などを用いて近接由来の変動を定量的に定義し、検証可能な形で根拠を与えた点である。曖昧になりがちな波及という現象を数理的に定義した上で説明生成に応用している。 第三に、数値的根拠に基づいて運用担当者が人流変動の原因を解釈し、対策判断につなげるための支援基盤として有効であることを、比較実験を通じて実証した点である。
\section{関連研究}
...
\begin{tabular}{l|l}
\Hline
需要推定（MSE） & 0.5075 (Last-hour) $\rightarrow$ 0.3673 (Ridge+GCN) \\
説明（+WEATHER+GCN条件） & HitRate 0.581，macro-F1 0.298 \\
\Hline
\end{tabular}
\end{center}
\end{table}
