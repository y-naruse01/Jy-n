...
\jtitle{気象情報と駅間共変動を用いた \GCN-\LLM\ Agent による鉄道旅客需要変動の説明生成}
\etitle{Explaining Railway Passenger Demand Fluctuations via a \GCN--\LLM\ Agent Integrating Weather and Inter-station Co-movement}
...
\begin{jabstract}
鉄道旅客需要予測では，数値的な需要変動だけでなく，その増減要因を運用担当者が理解可能な形で提示することが重要である。本研究では，駅間共変動に基づくグラフと気象情報を統合し，\GCN\ による需要推定と \LLM\ Agent による根拠付き説明生成を接続する枠組みを提案する。評価では，\GCN\ 特徴量により需要推定誤差（MSE）が0.5331から0.3673へ改善した。また，気象情報を根拠として与えることで，降雨・週末の要因検出で高いF1（0.718, 0.696）を得た。以上より，予測精度と説明の妥当性を同時に扱う枠組みの有用性を示す。
\end{jabstract}
...
\section{はじめに}
都市部の鉄道網は市民の移動を支える基幹インフラであり，ダイヤ調整，混雑緩和，要員配置といった高度な運用判断が日常的に求められる。近年，交通系ICカードの普及により駅単位・時間単位の乗降データが大量に蓄積されるようになり，LSTMやTransformerなどの系列モデル，さらに時空間グラフニューラルネットワーク（GNN）を用いた交通需要予測の研究が大きく進展した\cite{7508408,vaswani2017attention,10.5555/3304222.3304273}。こうした技術により，近未来の旅客需要を高精度に推定することは技術的に可能となってきた。

しかし，実運用においては予測値だけでは不十分な状況が存在する。改札制限の要否や駅員配置の増強を判断するためには，「1時間後に需要が増える」という数値に加えて，「近隣イベント終了に伴う流入か，降雨による他路線からの振替か」といった変動の理由を現場で即座に把握できることが不可欠である。鉄道旅客需要の変動は，降雨・気温などの外生的な気象要因と，乗換駅・隣接駅を介した駅間波及という二種類の要因が重畳して生じるため，単一駅の時系列のみから原因を特定することは困難である。既存の時空間GNNは駅間依存を考慮した高精度予測を実現しているが\cite{li2017diffusion,10.5555/3304222.3304273,wu2019graph,bai2020adaptive}，その予測根拠を自然言語で説明する機能を持たない。また，大規模言語モデル（LLM）による自然言語説明の試みも報告されているが\cite{guo2024towards}，LLMは一般知識に基づく補完が混入しやすく，実際の気象値や駅間変動といった観測事実と整合した説明を安定的に生成することが難しい。すなわち，気象要因・駅間依存・LLM説明生成の三者を統合し，観測事実を根拠として需要変動の要因を説明する枠組みは，これまで十分に構築されていない。

以上を踏まえ，本研究の目的は，気象要因と駅間依存を明示的な根拠として \LLM\ Agent に与えることで，鉄道旅客需要の増減要因を自然言語で説明できる統合フレームワークを構築し，その説明性能を定量的に検証することである。

この目的を達成するため，本研究では以下の3点を課題として設定する。第1に，需要変動の要因説明に必要な情報の理論的整理が不十分であり，気象条件・駅間共変動・旅客需要変動をいかに関連付けて定義・形式化するかという枠組みが求められる。既存研究では予測精度の向上が主目的とされており，「どの要因がどの程度寄与したか」を説明根拠として明示的に定義する試みは限られている。第2に，\GCN\ が抽出する近傍集約特徴量を \LLM\ Agent への根拠入力として接続する設計が確立されておらず，予測モデルと説明生成器を整合的に統合する方法論が必要である。従来のXAIでは予測モデルの出力を事後的に可視化する手法が主流であり，予測段階で得られた構造情報を説明生成へ直接再利用する枠組みは十分でない。第3に，生成説明の妥当性を定量評価する手法が未整備であり，特に気象要因・駅間波及といった鉄道需要固有の要因に対して，実世界を模した環境で実証的な検証を行う評価系の整備が求められる。

これらの課題に対し，本研究では次のアプローチをとる。まず，ICカード乗降データから駅間の共変動に基づくグラフを構築し，\GCN\ により近傍駅の変動を集約した特徴量を抽出する（課題1・2への対応）。次に，降雨量・気温・風速・週末フラグなどの気象特徴量と \GCN\ 近傍特徴量を候補根拠として \LLM\ Agent に入力し，許可ラベル集合で選択肢を限定することで根拠のない補完を抑制しながら要因説明を生成する（課題2への対応）。さらに，各要因の正解ラベルを付与可能な模擬データを用いた評価系を構築し，説明性能を定量的に検証する（課題3への対応）。本手法の核心は，\GCN\ の予測段階で得られた駅間依存の構造情報をそのまま説明根拠として \LLM\ Agent に渡す設計にある。これにより，予測と説明を同一パイプライン上で扱い，観測事実から逸脱しない説明の生成を可能にする。

本研究の貢献は以下の3点である。
\begin{itemize}
  \item \GCN\ による近傍集約特徴量と気象情報を根拠として \LLM\ Agent に提供し，観測事実に基づいた鉄道旅客需要変動の要因説明を生成する統合フレームワークを提案する。
  \item 要因ラベルを制御可能な模擬データと複数の定量指標（HitRate，macro-F1，NeighborAcc，ROUGE-L，BERTScore）を組み合わせた，鉄道需要説明に特化した評価系を構築する。
  \item 気象要因の統合が説明品質を大幅に改善すること（降雨 F1\,=\,0.718，週末 F1\,=\,0.696）を実験的に示す一方，駅間波及の説明には課題が残ること（NeighborAcc\,=\,0.000）を定量的に明らかにし，今後の改善指針を提示する。
\end{itemize}

本稿の構成は以下のとおりである。第2章で関連研究を整理し本研究の位置付けを明確にする。第3章では提案フレームワークの全体アーキテクチャと各構成要素を詳述する。第4章で実験設定と評価指標を述べ，第5章で実験結果を示す。第6章で考察を行い，第7章でむすびとする。

\section{関連研究}
本節では，はじめにで述べた「気象要因・駅間依存・\LLM\ 説明生成を観測事実に基づいて統合する」という問題設定に沿って関連研究を整理する。具体的には，(1) 需要変動を説明するための外生要因と駅間波及の扱い，(2) \GCN\ による駅間依存表現と説明生成への接続，(3) \LLM\ による根拠付き説明生成とその妥当性評価，の観点から既存研究を概観し，本研究の位置付けを明確にする。

\subsection{需要変動要因の表現と形式化}
交通需要予測の基盤として，RNNやLSTMに代表される系列モデルが広く用いられてきた。LSTMは長期依存の学習困難を緩和する構造として提案され\cite{7508408}，その性質や応用可能性も整理されている\cite{sherstinsky2020fundamentals,yu2019review}。また，Transformerは自己注意機構により系列内の長距離依存を並列的に扱う枠組みを確立した\cite{vaswani2017attention}。これらの系列モデルは周期性や過去の需要変動を表現する上で有効であるが，単一地点の履歴を中心とするため，鉄道網における駅間波及を直接的な説明要因として定義するには限界がある。

交通流は時間変化だけでなく空間的相互作用を伴うため，時空間モデルの重要性も早くから指摘されている\cite{kamarianakis2005space}。また，公共交通需要は降雨，気温，風などの気象条件から影響を受けることが報告されている\cite{zhou2017impacts}。したがって，鉄道需要変動を説明するためには，時系列履歴，気象条件，駅間共変動を同一の枠組みで扱う必要がある。しかし，既存研究の多くはこれらを予測精度向上のための入力特徴量として扱っており，気象値や近傍駅の変動を説明ラベルや根拠スニペットとして明示的に切り出し，「どの要因が需要変動の説明根拠となるか」を形式化する点は十分ではない。

\subsection{\GCN\ による駅間依存表現と説明生成への接続}
駅間依存を扱う手法として，グラフ構造上で特徴を集約する \GCN\ が基礎的枠組みを与えている\cite{kipf2016semi,defferrard2016convolutional}。交通予測では，DCRNNが拡散畳み込みと系列モデルを組み合わせて道路ネットワーク上の方向性を表現し\cite{li2017diffusion}，STGCNは時空間畳み込みにより交通流予測の性能を高めた\cite{10.5555/3304222.3304273}。さらに，AST-GCNは属性情報を統合することで，外部情報を含む交通予測の有効性を示している\cite{9363197}。これらの研究は，駅や道路区間をノードとみなし，近傍の状態を集約することが需要推定に有効であることを示している。

一方，鉄道需要では，物理的に隣接する駅だけでなく，乗換や利用パターンの類似によって離れた駅同士が連動する場合がある。この問題に対し，Graph WaveNetやAGCRNでは，固定グラフに依存せず適応的な隣接関係をデータから学習する手法が提案されている\cite{wu2019graph,bai2020adaptive}。また，GATは近傍ノードへの重みを学習できるため\cite{velickovic2017graph}，どのノードが予測に寄与したかを示す手がかりとなりうる。しかし，これらは主として予測モデル内部で駅間依存を利用する手法であり，得られた近傍集約特徴量を観測された気象条件とともに \LLM\ Agent へ渡し，自然言語の根拠付き説明へ接続する設計までは十分に扱っていない。

\subsection{根拠付き説明生成と定量評価}
予測モデルの説明可能性に関しては，深層学習と説明可能AIを組み合わせる研究が行われている\cite{monje2022deep}。また，近年は交通流予測に \LLM\ を用いて自然言語説明を付与する研究も報告されており\cite{guo2024towards}，数値予測を運用者が理解しやすい説明へ変換する可能性が示されている。しかし，\LLM\ は一般知識に基づく補完を行う傾向があるため，気象値や駅間変動といった観測事実を根拠として明示的に与えなければ，実際のデータと整合しない説明が生成される可能性がある。このため，説明生成では，モデル内部の重要度を事後的に示すだけでなく，入力時点で利用可能な根拠候補と生成される要因ラベルを対応付ける設計が重要となる。

生成説明の評価には，参照文との表層的一致を測るROUGE-L\cite{lin2004rouge}や，文脈表現に基づく意味的類似度を測るBERTScore\cite{scorato2023bertscore}などが広く用いられている。ただし，鉄道需要説明では，文章として自然であることだけでなく，降雨，高温，強風，週末，駅間波及といった要因ラベルが観測事実と一致しているかを評価する必要がある。したがって，汎用的な生成文評価に加えて，HitRate，macro-F1，NeighborAccのようなドメイン固有の要因一致指標を用いる評価系が求められる。

以上より，既存研究は時系列需要予測，時空間GNN，外生要因統合，説明可能AI，\LLM\ による説明生成の各領域で進展している。しかし，気象条件・駅間共変動・需要変動を説明根拠として定義し，\GCN\ による近傍集約特徴量を観測事実に基づく根拠として \LLM\ Agent へ接続し，さらに鉄道需要固有の要因一致に基づいて説明性能を定量評価する枠組みは十分に整備されていない。本研究は，はじめにで設定した三つの課題に対応し，予測と説明を同一パイプラインで扱う点に特徴がある。
...
提案フレームワークは，(1) 入力データ整備，(2) 共変動に基づくグラフ構築，(3) \GCN\ による需要推定，(4) \LLM\ Agent による根拠付き説明生成，の4段階から成る。
\begin{figure}[t]
  \centering
  \fbox{\parbox{0.90\linewidth}{\centering
    \Large 本研究の全体アーキテクチャ図\\[1em]
    （図1：提案フレームワークの概要）\\
    入力データ整備$\rightarrow$共変動グラフ構築$\rightarrow$\GCN\ による需要推定$\rightarrow$\LLM\ Agent\ による根拠付き説明生成
  }}
...
需要推定では，時間特徴のみの線形モデル（Linear only）と，\GCN\ 由来の近傍集約特徴を追加したモデル（Linear + GCN-feats）を比較する。\GCN\ は予測器というより，構築したグラフ上で近傍情報を集約する特徴抽出器として利用する。損失関数には，駅平均利用量 $\mu_i$ に基づく重み付きMSEを用いる。
\begin{equation}
...
\subsection{提案手法の特徴}
本提案手法の最大の特徴は，予測モデルと説明生成器を密に統合することにある。既存の説明可能AI (XAI) では，予測モデルの内部状態を後から可視化する手法が主流であり，その説明は往々にして抽象的・一般論的になりがちである。一方，本研究では，\GCN\ によって抽出された近傍特徴を直接 \LLM\ Agent に供給することで，「駅Aで急増したため，隣接駅Bにも波及した」といった具現的かつ因果的な説明を可能にする。この設計により，運用担当者が直ちに検証可能な根拠を伴った説明が得られ，予測値の信頼性向上と運用判断の精度向上に寄与する。
...
\section{実験}
本章では，提案手法の有効性を，予測性能と説明性能の両面から検証する。評価は三つの観点から行った。第一に，\LLM\ Agent に外部要因を与えることで説明性能が改善するかを確認した。第二に，共変動に基づくグラフと \GCN\ により需要推定精度が向上するかを評価した。第三に，気象情報と駅間波及情報の統合が，説明の妥当性にどのように寄与するかをアブレーションで検証した。
...
\subsection{構築グラフの診断}
構築したグラフの妥当性を確認するため，ノード数，エッジ数，平均次数に加え，エッジ上の平均類似度，ラグ付き相互相関，最大相関が $\pm1$ band に収まる比率を測定した。さらに，次数上位ノードを除去した hub ablation により，主要駅欠損時の頑健性も確認した。
...
\section{結果}
本章では，グラフの構造妥当性，需要推定性能，および \LLM\ Agent による説明生成性能を順に示す。特に，\GCN\ 特徴量の追加が需要推定誤差に与える影響，ならびに気象要因と駅間波及情報の提示が \LLM\ による説明の妥当性にどの程度寄与するかに着目して結果を整理する。

\subsection{構築グラフの妥当性}
提案フレームワークに基づき構築したグラフは，1451ノード，13243エッジ，平均次数18.25となった。エッジ上の平均類似度は0.868，ラグ付き相互相関の平均は0.869であり，最大相関が $\pm1$ band に収まる比率は0.998であった。これは，結合駅間の変動がほぼ同期的であり，\GCN\ の近傍集約仮定と整合することを示している。
...
\section{考察}
需要推定の結果より，単駅の自己回帰による予測（Last-hour, Ridge）には限界があるが，\GCN\ による近傍特徴を加えることで誤差が大幅に減少した。これは，鉄道需要の短期変動が単駅の自己履歴だけでなく，周辺駅の同時変動と強く結び付いていることを示している。したがって，本研究で構成したグラフは，説明生成以前の予測段階においても有効な構造表現となっている。

また，ネットワーク診断結果では，エッジ上の平均類似度およびラグ付き相互相関が高く，最大相関がほぼ $\pm1$ band に収まった。これは，構築したグラフが駅間の共変動を妥当に表現しており，\GCN\ の近傍集約仮定と整合していることを意味する。単なる地理隣接ではなく，利用パターンの連動を表す構造を採用したことが，予測と説明の両面で効いていると考えられる。
...
\section{むすび}
本稿では，共変動に基づくグラフと気象情報を統合し，\GCN\ による需要推定と \LLM\ Agent による根拠付き説明生成を統一的に扱う枠組みを提案した。このグラフの近傍特徴を取り込むことで，需要推定誤差を0.5331から0.3673へ改善した。また，気象要因および駅間波及情報を説明生成へ与えることで，降雨・週末の要因検出で高いF1（0.718, 0.696）を達成した。提案手法は，予測精度向上と説明の妥当性を同時に実現する新枠組みとして，鉄道運用支援分野への応用が期待される。今後は，実データによる検証，外部要因の拡張，説明整合性の自動検証を進める。
